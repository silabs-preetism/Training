...
void RSI_IPMU_UpdateIpmuCalibData(efuse_ipmu_t *ipmu_calib_data)
{
  ...
  if (retention_reg->product_mode == MCU) {
    //ROW 38
    RSI_IPMU_UpdateIpmuData((NWP_AHB_ADDR + 0x604),
                            DIRECT,
                            (ipmu_calib_data->scale_soc_ldo_vref << 31),
                            MASK_BITS(1, 31));
  }
  //ROW 4,5,6
  data = (ipmu_calib_data->trim_0p5na1 << 18) | (ipmu_calib_data->trim_0p5na2 << 19)
         | (((ipmu_calib_data->bg_r_vdd_ulp >> 3) & 0x1) << 12) | (ipmu_calib_data->resbank_trim << 10);
  ...
  //ROW 9
  RSI_IPMU_UpdateIpmuData(ULPCLKS_DOUBLER_XTAL_REG_OFFSET,
                          ULP_SPI,
                          (ipmu_calib_data->del_2x_sel << 15),
                          MASK_BITS(6, 15));
  ...
  //ROW 13
  RSI_IPMU_UpdateIpmuData(ULPCLKS_HF_RO_CLK_REG_OFFSET,
                          ULP_SPI,
                          (ipmu_calib_data->trim_ring_osc << 14),
                          MASK_BITS(7, 14));
...
/**
 * @fn          error_t RSI_APB_ProgramConfigData(uint32_t *config)
 * @brief	    This API is used to program the any mcu configuration structure
 * @return      execution status.
 */
...
/**
 * @fn         error_t RSI_IPMU_PocbiasCcurrent11(void)
...
/**
 * @fn        error_t RSI_IPMU_RetnLdoLpmode(void)  
...
/**
 * @fn          uint32_t RSI_IPMU_RO_TsEfuse(void)
...
/**
 * @fn          error_t RSI_IPMU_Auxadcoff_SeEfuse(void)
 ...
 * @return    returns trim value for required MHz RC frequency which we can reserve for further usage  .
 */
...
/**
 * @fn          void RSI_20M_ROClktrim(uint8_t clkfreq)
...
/**
 * @fn        uint32_t uint32_t RSI_Clks_Calibration(INPUT_CLOCK_T inputclk,SLEEP_CLOCK_T sleep_clk_type)
