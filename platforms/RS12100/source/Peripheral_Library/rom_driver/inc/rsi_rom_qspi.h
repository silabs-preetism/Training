...
#ifdef CHIP_9117
/**
 *  @fn      STATIC INLINE void RSI_QSPI_QspiLoadKey(qspi_reg_t *qspi_reg, int8_t mode, uint32_t *key, uint32_t kh_enable)
 *  @brief   This function is used to load the AES key to QSPI Controller.
 *  @param   qspi_reg        : Pointer to qspi_reg_t structure,contains all qspi registers \ref qspi_reg_t
 *  @param   key  					 : Pointer to the key
 ...
 *  @return  none
 */
STATIC INLINE void RSI_QSPI_QspiLoadKey(qspi_reg_t *qspi_reg, int8_t mode, uint32_t *key, uint32_t kh_enable)
{
  //TODO : Need to add this API in ROM
