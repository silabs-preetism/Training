...
/* ***************************************************************************
 * Function:puf_block(through misc_cfg_reg)
 * Description:Blocks the operation according to the given block type
 * Input:block type
 * output:none
...
/* ***************************************************************************
 * Function: puf_wait_for_init
 * Description:Wait for initialization of PUF after power up QK_SR
...
/* ***************************************************************************
 * Function:puf_enroll
 * Description:generates the activation code and can be read through PUF o/p registers
...
/* ***************************************************************************
 * Function:puf_start
 * Description:In the start operation,the given activation code is
 * loaded to the PUF
 * Input parameters:pointer of activation code
 * return: Zero: if success
 * 		   one : if error
 *************************************************************************/
...
/* ***************************************************************************
 * Function: puf_set_uk
 * Description: For the given User key,key code of 11*4 bytes is generated which can be stored in
 * 				NVM
 * Input parameters:	1.index of the key
 * 						2.user key
 * 						3.key size(128/256)
 * Output:	Key code for the corresponding User key
 * return: Zero: if success
 * 		   one : if error
 *************************************************************************/
...
/* ***************************************************************************
 * Function: puf_get_key
 * Description:	For the given key code, key is generated
 ...
 * Input parameters:	1.key code
 * 						2.interface mode(0x00=>interface;0x01=>register;0x02 => keyholder)
 * Output: key output if interface mode is Register
 * return: Zero: if success
 * 		   one : if error
 *************************************************************************/
...
/* ***************************************************************************
 * Function: data_aes_mac
 * Description:This function is used to calculate MAC for the given bytes
 * Input parameters:1. Mode (Reserved for now)
 * 							  B1 : 1- Key Size 256, 0- Key size 128;
 * 							  B3 : 1-IV Size 256, 0- IV size 128;
 *	 	 	 	 	2. Pointer to plain data
 *	 	 	 	 	3. Pointer to IV
 *	 	 	 	 	4. Pointer to AES key
 *	 	 	 	 	5. Plain data size(in Bytes)
 *
 * Output : 1. Pointer to Calculated MAC
 * return : Zero		: if success
 * 		    Non-Zero    : if error
 *************************************************************************/
...
/* ***************************************************************************
 * Function: data_aes_encry_decry
 * Description:This function is used to Encrypt/Decrypt using AES engine either
 *  in CBC/ECB mode
 * Input parameters:1. Mode   B0 : 1 - CBC, 0 - ECB;
 * 							  B1 : 1 - Key Size 256, 0- Key size 128;
 ...
 *	 	 	 	 	5. Plain data size(in Bytes)
 *
 * Output : 1. Pointer to Output Data
 * return : Zero		: if success
 * 		    Non-Zero    : if error
 *************************************************************************/
...
/* ***************************************************************************
 * Function: puf_set_intrensic_key
 * Description:This function is used to generate key code for the intrinsic key
 * Input parameters:1.key index
 * 					2.key size (128/256)
 * Output:key code
 * return: Zero: if success
 * 		   one : if error
 ...
 *
 *	 	 	 	 	2. Pointer to Input data
 *	 	 	 	 	3. Input data offset in flash
 *	 	 	 	 	4. Pointer to IV
 *	 	 	 	 	5. Pointer to AES key
 *	 	 	 	 	6. Plain data size(in Bytes)
 *
 * Output : 1. Pointer to Output Data
 * return : Zero		: if success
 * 		    Non-Zero    : if error
