 ...
 * Project:      CAN Driver for Redpine MCU
 * --------------------------------------------------------------------------
...
static int32_t CAN_PowerControl (ARM_POWER_STATE state) {
   switch (state) {
     ...
     case ARM_POWER_FULL:
       ...
       M4CLK->CLK_ENABLE_SET_REG3 = M4_SOC_CLK_FOR_OTHER_ENABLE; // Enable common clock for peripheral on which CAN is also workin
...
static int32_t CAN_SetBitrate (ARM_CAN_BITRATE_SELECT select, uint32_t bitrate, uint32_t bit_segments) {
  ...
  if(brp > 64U)      { return ARM_CAN_INVALID_BITRATE; }
