...
static int32_t PowerControl (ARM_POWER_STATE state) {
  ...
  switch (state) {
    ...
    case ARM_POWER_FULL:
      ...
      ETH->MAC_ADDR0_HIGH_REG = 0U; ETH->MAC_ADDR0_LOW_REG = 0U;
      ETH->MAC_ADDR1_HIGH_REG = 0U; ETH->MAC_ADDR1_LOW_REG = 0U;
      ETH->MAC_ADDR2_HIGH_REG = 0U; ETH->MAC_ADDR2_LOW_REG = 0U;
      ETH->MAC_ADDR3_HIGH_REG = 0U; ETH->MAC_ADDR3_LOW_REG = 0U;
...
static int32_t SetAddressFilter (const ARM_ETH_MAC_ADDR *ptr_addr, uint32_t num_addr) {
  ...
  ETH->MAC_HASH_TABLE_HIGH_REG = 0U; ETH->MAC_HASH_TABLE_LOW_REG = 0U;
...
static int32_t Control (uint32_t control, uint32_t arg) {
  ...
  switch (control) {
    case ARM_ETH_MAC_CONFIGURE:
		maccr = ETH->MAC_CONFIG_REG & ~( ETH_MACCR_DM   |
                             ETH_MACCR_LM  | ETH_MACCR_IPC);
