...
#ifndef __RSI_UDMA_H__
#define __RSI_UDMA_H__
...
typedef struct {
  ...
  volatile RSI_UDMA_CHA_CONFIG_DATA_T
    vsUDMAChaConfigData1;  /*!< The Channel Config Structure                           */
 ...
 * @brief		    This API is used to Get memory size in bytes needed for UDMA 
                controller driver context
 ...
 * @brief		    This API is used to to generate a software UDMA request on the 
                corresponding UDMA channel 
 * @param[in]	  pHandle	:  Pointer to driver context handle
 * @param[in]	  dmaCh   : channel number (0 to 31).
 * @return         return ERROR_UDMA_INVALID_ARG if channel is  greater than 31 or channel is invalid , and on success
                           return RSI_OK(0).
 */
