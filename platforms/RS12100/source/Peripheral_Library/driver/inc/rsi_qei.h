...
#ifndef __RSI_QEI_H__
#define __RSI_QEI_H__
#ifdef __cplusplus
extern "C" {
#endif
...
#define QEI_POS_CNT_ERR_INTR_LVL \
  BIT(                           \
    2) /*!< Whenever number of possible positions are mismatched with actual positions are received between two index pulses this will raised. */
#define QEI_VELO_LESS_THAN_INTR_LVL \
  BIT(                              \
    3) /*!< When velocity count is less than the value given in velocity_value_to_compare register, interrupt is raised. */
...
#define QEI_IDX_CNT_RST_EN \
  BIT(8) /*!< 1= index counter is going to reset after reaching max count, which is mentioned in*/
...
#define QEI_TIMER_MODE \
  BIT(                 \
    10) /*!< 1 = timer mode. In this mode, decoded timer pulse and direction are taken from position A and position B pins respectively. */
...
/**
 * \defgroup RSI_QEI_DRIVERS RSI:RS1xxxx QEI 
 *  @{
 *
 */
/**
