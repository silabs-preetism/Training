 ...
 * @param  adcChConfig  : ADC channels configuration structure variable.
 * @param  adcConfig    : ADC operation configuration structure variable. 
 * @param  event        : Callback function pointer.
 * @retval error_t      : Execution status,This function return 'RSI_OK' on
...
#ifndef ADC_MULTICHANNEL_ENABLE
 ...
 * @param  ch_num       : ADC Channel number (0 to 15).
 * @retval error_t      :Returns the channel sampling rate
 */
 ...
 * @param  event        : EXTERNAL_DMA_RECONFIG or INTERNAL_DMA          
 * @retval error_t      : Execution status,This function return 'RSI_OK' on
 *                        successful execution.  
 */
 ...
 * @param  data_select : This parameter define ADC data is available in ping location or pong location.
 ...
 * @param  num      : Given number to find out number is power of two or not power of two. 
 * @retval Return the status of given number is power of two or not power of two.
 ...
 * @param  num      : Given number to round-off with nearest power of two number. 
 * @retval Return nearest power of two number. 
 */
 ...
 * @param  number_of_channel : Number of channel enable for ADC operation.
 * @retval ADC effective sampling rate. 
 */
 ...
 * @param  number_of_channel : Number of channel enable for ADC operation.
 * @retval Required minimum sampling time(ON time of ADC cycles)  
 */
 ...
 * @param  pos_inp_sel : Positive input selection for ADC channel.
 * @param  neg_inp_sel : Negative input selection for ADC channel.
 * @param  input_type  : Select single ended or differential ended input to ADC, 
...
error_t ADC_PinMux(uint8_t pos_input_pinsel, uint8_t neg_input_pinsel, uint8_t input_type)
{
  ...
  if ((pos_input_pinsel == POS16) || ((neg_input_pinsel == POS6) && (input_type == POS1))) {
    *(volatile uint32_t *)(SDIO_HOST_CONTROL_IN_M4) = BIT(5);
    ...
    RSI_EGPIO_SetPinMux(EGPIO, 0, ADC_TOPGPIO26, ANALOG_MODE);
  }
  if ((pos_input_pinsel == POS17) || ((neg_input_pinsel == POS7) && (input_type == POS1))) {
    *(volatile uint32_t *)(SDIO_HOST_CONTROL_IN_M4) = BIT(5);
    ...
    RSI_EGPIO_SetPinMux(EGPIO, 0, ADC_TOPGPIO28, ANALOG_MODE);
  }
  if ((pos_input_pinsel == POS18) || ((neg_input_pinsel == POS8) && (input_type == POS1))) {
    *(volatile uint32_t *)(SDIO_HOST_CONTROL_IN_M4) = BIT(5);
    ...
    RSI_EGPIO_SetPinMux(EGPIO, 0, ADC_TOPGPIO30, ANALOG_MODE);
  }
  if ((pos_input_pinsel == POS19) || ((neg_input_pinsel == POS9) && (input_type == POS1))) {
    RSI_EGPIO_PadReceiverDisable(ADC_TOPGPIO24);
    RSI_EGPIO_SetPinMux(EGPIO, 0, ADC_TOPGPIO24, ANALOG_MODE);
  }
  return RSI_OK;
}
...
#endif
#ifdef CHIP_9117
  pstcADC->ADC_FIFO_THRESHOLD_b.ADC_FIFO_AEMPTY_THRESHOLD = a_empty_threshold;
  pstcADC->ADC_FIFO_THRESHOLD_b.ADC_FIFO_AFULL_THRESHOLD  = a_full_threshold;
#endif
                                    ...
                                    uint16_t adc_ch_freq_val)
{
  if (adc_ch_freq_val == 1) {
    ...
    pstcADC->ADC_CH_FREQ_b[channel].CH_FREQ_VALUE = adc_ch_freq_val;
  }
  return RSI_OK;
}
 ...
 * @return       execution status 
 * @note         minimum ON time is 15us if opamp comes into picture.
 ...
 * adc_total_duration = total clock duration i.e adc_on_time + adc_off time duration. (15 + 1) clocks
 */
 ...
 *
 */
error_t RSI_ADC_ThresholdConfig(AUX_ADC_DAC_COMP_Type *pstcADC, uint32_t threshold1, uint32_t threshold2, uint8_t range)
{
  int16_t temp_1                                                = 0;
...
#ifdef CHIP_9117
    RSI_ADC_ChnlIntrMask(AUX_ADC_DAC_COMP, 0, 1);
#endif
 ...
 * @param  udmaHandle  :  uDMA handler
 * @param  pTranDesc   :  Event type 
 * @retval None
 */
 ...
 * @param  reconfig : This parameter use for skip uDMA initialization function in 
