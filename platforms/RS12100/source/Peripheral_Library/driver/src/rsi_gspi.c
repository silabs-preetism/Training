                        ...
                        uint32_t *mem)
{
  ...
  if ((gspi->reg == GSPI0)) {
    ...
    //configure cs pin
    if (gspi->io.cs->pin > 63) {
      RSI_EGPIO_UlpPadReceiverEnable((uint8_t)(gspi->io.cs->pin - 64));
      RSI_EGPIO_SetPinMux(EGPIO1, 0, (uint8_t)(gspi->io.cs->pin - 64), 6);
    }
    if (gspi->io.cs->pad_sel != 0) {
      RSI_EGPIO_PadSelectionEnable(gspi->io.cs->pad_sel);
    }
    if (gspi->io.cs->pin >= 25 && gspi->io.cs->pin <= 30) {
      RSI_EGPIO_HostPadsGpioModeEnable(gspi->io.cs->pin);
    }
    RSI_EGPIO_PadReceiverEnable(gspi->io.cs->pin);
    RSI_EGPIO_SetPinMux(EGPIO, gspi->io.cs->port, gspi->io.cs->pin, gspi->io.cs->mode);
    ...
    case ARM_SPI_CONTROL_SS:
      ...
      if (arg == ARM_SPI_SS_INACTIVE) {
        if (val == ARM_SPI_SS_MASTER_HW_OUTPUT) {
          ...
          RSI_EGPIO_SetPin(EGPIO, gspi->io.cs->port, gspi->io.cs->pin, 1);
        }
      } else {
        if (val == ARM_SPI_SS_MASTER_HW_OUTPUT) {
          /* Assert selected CS line */
          ...
          RSI_EGPIO_SetPin(EGPIO, gspi->io.cs->port, gspi->io.cs->pin, 0);
        }
      }
      return ARM_DRIVER_OK;
  }
  ...
  if ((gspi->info->mode & ARM_SPI_CONTROL_Msk) == ARM_SPI_MODE_MASTER) {
    switch (control & ARM_SPI_SS_MASTER_MODE_Msk) {
      case ARM_SPI_SS_MASTER_UNUSED: // SPI Slave Select when Master: Not used (default)
        ...
        RSI_EGPIO_SetPinMux(EGPIO, gspi->io.cs->port, gspi->io.cs->pin, 0);
      ...
      case ARM_SPI_SS_MASTER_SW: // SPI Slave Select when Master: Software controlled
        ...
        RSI_EGPIO_PadSelectionEnable(gspi->io.cs->pad_sel);
        RSI_EGPIO_SetPinMux(EGPIO, gspi->io.cs->port, gspi->io.cs->pin, 0); //gpio mode
        RSI_EGPIO_SetDir(EGPIO, gspi->io.cs->port, gspi->io.cs->pin, 0);    //set output direction
        RSI_EGPIO_SetPin(EGPIO, gspi->io.cs->port, gspi->io.cs->pin, 1);    //set pin value to 1
                  ...
                  RSI_UDMA_HANDLE_T udmaHandle)
{
  ...
  if (gspi->tx_dma) {
    if (num > 10) {
      ...
      gspi->reg->GSPI_FIFO_THRLD_b.FIFO_AEMPTY_THRLD = gspi->threshold->gspi_aempty_threshold;
      gspi->reg->GSPI_FIFO_THRLD_b.FIFO_AFULL_THRLD  = gspi->threshold->gspi_afull_threshold;
      ...
      control.rPower      = gspi->threshold->txdma_arb_size;
  ...
  if ((gspi->rx_dma) || (gspi->tx_dma)) {
    if (num > 10) {
      ...
      if ((gspi->tx_dma)) {
        ...
        control.rPower      = gspi->threshold->rxdma_arb_size;
        control.srcProtCtrl = 0x0;
        control.dstProtCtrl = 0x0;
        if ((data_bits <= 8) && (data_bits != 0)) {
          //8-bit data frame
