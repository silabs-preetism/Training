 ...
 * @param[in]    channel_selection_value     : This parameter define on which channel frequency use
 *                                             for transmission . 
 ...
 *                                             Note : Refer  RSI_WURX_CalVCOCalFreq() API ,
 *                                                           its return calculated value.   
 * @return       None
 */
 ...
 * @Example      This API get the tail data of receive pattern,maximum tail data is 256 bits.
 *               E.g uint32_t tail_data[8];  
 *                   RSI_WURX_GetTailData(tail_data,TAIL_DATA_DECODE_256BIT);
 *                   So in above tail_data buffer contain the tail data as below:
 *                   tail_data[0] : LSB of pattern
 *                   -
 *                   - 
 *                   tail_data[7] : MSB of pattern  
 *                 
 */
 ...
 * @param[in]    None
 * @return       Return the number which contain how much data we want to read in tail data e.g if return 0 then 64bit , if 1 then 128 bit and so on. 
 */
...
/**
 * @fn           uint16_t RSI_WURX_GetPatternType(void)
 * @brief        This API is use to get pattern type . e.g pattern1 or pattern2 or false wakeup. 
 * @param[in]    None
 * @return       Pattern type .if its return 1 then pattern1 and 2 then pattern2 , if -1 then false wakeup.
 */
 ...
 * @return       Return the VCO calibration frequency value. 
 */
