...
#elif USE_M4_MEMORY
  /* M4SS Memory buffers from application */
  FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrc >> M4SS_ADDR_SHIFT_VAL;
  FIM->FIM_OUT_ADDR_b.OUT_ADDR = (uint32_t)pDst >> M4SS_ADDR_SHIFT_VAL;
#endif
#if USE_ULPSS_MEMORY //ULPSS buffers from application
...
#elif USE_M4_MEMORY
  /* M4SS Memory buffers from application */
  FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrcA >> M4SS_ADDR_SHIFT_VAL;
  FIM->FIM_INP2_ADDR_b.INP2_ADDR = (uint32_t)pSrcB >> M4SS_ADDR_SHIFT_VAL;
  FIM->FIM_OUT_ADDR_b.OUT_ADDR = (uint32_t)pDst >> M4SS_ADDR_SHIFT_VAL;
#endif
#if USE_ULPSS_MEMORY
  /* ULPSS buffers from application */
...
#elif USE_M4_MEMORY
  /* M4SS Memory buffers from application */
  FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pIn1 >> M4SS_ADDR_SHIFT_VAL;
  FIM->FIM_INP2_ADDR_b.INP2_ADDR = (uint32_t)pIn2 >> M4SS_ADDR_SHIFT_VAL;
  FIM->FIM_OUT_ADDR_b.OUT_ADDR = (uint32_t)pDst >> M4SS_ADDR_SHIFT_VAL;
#endif
#if USE_ULPSS_MEMORY
  /* ULPSS buffers from application */
...
#elif USE_M4_MEMORY
  /* M4SS Memory buffers from application */
  FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrcCmplx >> M4SS_ADDR_SHIFT_VAL;
  FIM->FIM_INP2_ADDR_b.INP2_ADDR = (uint32_t)pSrcReal >> M4SS_ADDR_SHIFT_VAL;
  FIM->FIM_OUT_ADDR_b.OUT_ADDR = (uint32_t)pDst >> M4SS_ADDR_SHIFT_VAL;
#endif
#if USE_ULPSS_MEMORY
  /* ULPSS buffers from application */
...
#elif USE_M4_MEMORY
      /* M4SS Memory buffers from application */
      FIM->FIM_INP1_ADDR_b.INP1_ADDR = ((uint32_t)pSrcA->pData) >> M4SS_ADDR_SHIFT_VAL;
      FIM->FIM_INP2_ADDR_b.INP2_ADDR = ((uint32_t)pSrcB->pData) >> M4SS_ADDR_SHIFT_VAL;
      FIM->FIM_OUT_ADDR_b.OUT_ADDR = ((uint32_t)pDst->pData) >> M4SS_ADDR_SHIFT_VAL;
#endif
#if USE_ULPSS_MEMORY
      /* ULPSS buffers from application */
                         ...
                         uint16_t outBank)
{
  ...
  /* M4SS Memory buffers from application */
  FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrc >> M4SS_ADDR_SHIFT_VAL;
  FIM->FIM_INP2_ADDR_b.INP2_ADDR = (uint32_t)S->pCoeffs >> M4SS_ADDR_SHIFT_VAL;
  FIM->FIM_OUT_ADDR_b.OUT_ADDR = (uint32_t)pDst >> M4SS_ADDR_SHIFT_VAL;
#endif
#if USE_ULPSS_MEMORY
  /* ULPSS buffers from application */
...
#ifdef ENHANCED_FIM //Add comment list of new features /Ravi
...
#ifdef ULPSS_MEMORY_WITH_M4_MEM_BUFFRS
  /* Use application buffer which are in M4 RAM and copy them to ulpss RAM ,point ulpss RAM address to FIM */
...
#else
  /*Check  */
  if ((((uint32_t)pSrcA) & (ULPSS_BASE_ADD_MASK_VAL)) == ULPSS_BASE_ADD) {
    /* ULPSS buffers from application */
    /* Start Address of 1st Input Data for FIM Operations  */
    FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrcA;
    /* Start Address of 2nd Input Data for FIM Operations  */
...
void arm_fir_decimate_q15(const arm_fir_decimate_instance_q15 *S, q15_t *pSrc, q15_t *pDst, uint32_t blockSize)
{
  ...
  FIM->FIM_OUT_ADDR_b.OUT_ADDR = ULPSS_RAM_ADDR_DST;
#else
  if (((uint32_t)pSrc & (ULPSS_BASE_ADD_MASK_VAL)) == ULPSS_BASE_ADD) {
    /* ULPSS buffers from application */
    /* Start Address of 1st Input Data for FIM Operations  */
    FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrc;
    ...
    /* M4SS Memory buffers from application */
    /* Start Address of 1st soruce data for FIM Operations  */
    FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrc >> M4SS_ADDR_SHIFT_VAL;
    /* Start Address of 1st coefficient for FIM Operations  */
    FIM->FIM_INP2_ADDR_b.INP2_ADDR = (uint32_t)S->pCoeffs >> M4SS_ADDR_SHIFT_VAL;
    /* Start Address of Output Data for FIM Operations */
...
void rsi_arm_cfft_radix2(q31_t *pSrc)
{
  ...
  if (((uint32_t)pSrc & (ULPSS_BASE_ADD_MASK_VAL)) == ULPSS_BASE_ADD) {
    /* ULPSS buffers from application */
    /* Start Address of even address  Input Data for FIM Operations  */
    FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrc_even;
    /* Start Address of  odd address for FIM Operations  */
    FIM->FIM_INP2_ADDR_b.INP2_ADDR = (uint32_t)pSrc_odd;
    /* Start Address of twiddle factor Data for FIM Operations */
    FIM->FIM_OUT_ADDR_b.OUT_ADDR = (uint32_t)TWIDDLE_FACTOR_TABLE;
  } else {
    /* M4SS Memory buffers from application */
    /* Start Address of even address  Input Data for FIM Operations  */
    FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrc_even >> M4SS_ADDR_SHIFT_VAL;
    /* Start Address of  odd address for FIM Operations  */
    FIM->FIM_INP2_ADDR_b.INP2_ADDR = (uint32_t)pSrc_odd >> M4SS_ADDR_SHIFT_VAL;
    /* Start Address of twiddle factor Data for FIM Operations */
    FIM->FIM_OUT_ADDR_b.OUT_ADDR = (uint32_t)TWIDDLE_FACTOR_TABLE >> M4SS_ADDR_SHIFT_VAL;
  }
#endif
  /* Set the Mode of Operation to be performed. */
...
void rsi_arm_sin_q15(q15_t *pSrc, q15_t *pDst, uint32_t blocksize)
{
  ...
  if (((uint32_t)pSrc & (ULPSS_BASE_ADD_MASK_VAL)) == ULPSS_BASE_ADD) {
    ...
    FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrc >> M4SS_ADDR_SHIFT_VAL;
    /* Start Address of lookup table Data for FIM Operations  */
    FIM->FIM_INP2_ADDR_b.INP2_ADDR = (uint32_t)Trig_lut1 >> M4SS_ADDR_SHIFT_VAL;
    /* Start Address of Output Data for FIM Operations */
    FIM->FIM_OUT_ADDR_b.OUT_ADDR = (uint32_t)pDst >> M4SS_ADDR_SHIFT_VAL;
  }
#endif
...
void rsi_arm_Sinh_q15(q15_t *pSrc, q15_t *pDst, uint32_t blocksize)
{
  ...
  if (((uint32_t)pSrc & (ULPSS_BASE_ADD_MASK_VAL)) == ULPSS_BASE_ADD) {
    ...
    FIM->FIM_INP1_ADDR_b.INP1_ADDR = (uint32_t)pSrc >> M4SS_ADDR_SHIFT_VAL;
    /* Start Address of lookup table Data for FIM Operations  */
    FIM->FIM_INP2_ADDR_b.INP2_ADDR = (uint32_t)Trig_lut2 >> M4SS_ADDR_SHIFT_VAL;
    /* Start Address of Output Data for FIM Operations */
    FIM->FIM_OUT_ADDR_b.OUT_ADDR = (uint32_t)pDst >> M4SS_ADDR_SHIFT_VAL;
  }
#endif
...
error_t arm_mat_trans_q15(const arm_matrix_instance_q15_opt *pSrc, arm_matrix_instance_q15_opt *pDst)
{
  ...
  if (pSrc == NULL) {
    ...
    if (((uint32_t)pSrc & (ULPSS_BASE_ADD_MASK_VAL)) == ULPSS_BASE_ADD) {
      ...
      /* M4SS Memory buffers from application */
      /* Start Address of 1st Input Data for FIM Operations  */
      FIM->FIM_INP1_ADDR_b.INP1_ADDR = ((uint32_t)pSrc->pData) >> M4SS_ADDR_SHIFT_VAL;
      /* Start Address of Output Data for FIM Operations */
      FIM->FIM_OUT_ADDR_b.OUT_ADDR = ((uint32_t)pDst->pData) >> M4SS_ADDR_SHIFT_VAL;
    }
#endif
