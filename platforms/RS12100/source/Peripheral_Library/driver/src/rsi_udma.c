 ...
 * @brief		    This API is used to get version information of the driver 
								implementation
 * @param[in]	  none
 * @return 		  structure of type RSI_DRIVER_VERSION and its members are as below
								  - \ref RSI_UDMA_API_VERSION : Version of the CMSIS-Driver specification 
																			 used to implement this driver.
								  - \ref RSI_UDMA_DRV_VERSION : UDMA peripheral source code version of the 
 ...
 * @brief		    The function RSI_UDMA_GetCapabilities returns information about 
								capabilities of this driver implementation.
 * @param[in]	  none
 * @return 		  structure of type RSI_UDMA_CAPABILITIES and its members are as below
 ...
 * @return 		  return ERROR_UDMA_INVALID_ARG if channel is  greater than 31, and on success return RSI_OK(0).
 */
...
/*This API is used to control parameters for a UDMA channel control structure.*/
                                    ...
                                    void *pDstAddr)
{
  ...
  if ((channelTableIndex & 0xffff) < 64) {
    ...
    /* Set transfer length */
    if (/*(vsUdmaChaConfigData.totalNumOfDMATrans >= 0)&& */
        (vsUdmaChaConfigData.totalNumOfDMATrans <= UDMA_CHCTL_XFERSIZE_M)) {
    ...
    /* Set arbitration */
    if (/*(vsUdmaChaConfigData.rPower >= 0) && */
        (vsUdmaChaConfigData.rPower <= ARBSIZE_1024)) {
...
/* This API is used to configures a UDMA channel for scatter-gather mode */
...
/**
 * @fn          void RSI_UDMA_AckEnable(RSI_UDMA_HANDLE_T pHandle,uint32_t peripheral)
...
/**
 * @fn          void udma_deInit(RSI_UDMA_HANDLE_T pHandle, RSI_UDMA_CHA_CFG_T *pCfg)
...
/*This API is used to handle all interrupt and error flags in interrupt context */
...
const ROM_UDMA_API_T udma_api = {
		&udma_init,
		&udma_get_channel_transfer_mode,
		&udma_setup_channel_transfer,
		&udma_set_channel_scatter_gather_transfer,
		&udma_get_channel_transfer_length,
		&udma_setup_channel,
		&udma_deInit,
		&udma_interrupt_handler,
		&udma_interrupt_enable,
};  
