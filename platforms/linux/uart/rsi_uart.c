...
void rsi_recv_pkt_serial()
{
  ...
  while (rsi_serial_fifo_status())
  {
    ...
    if (rsi_linux_app_cb.rx_is_in_progress == RSI_UART_PAYLOAD_RECV_STATE) {
      ...
      if((rsi_linux_app_cb.rcvPktPtr->desc[2] == 0xA4)&&(rsi_linux_app_cb.rcvPktPtr->desc[1] == 0x40))
      {
	  if(rsi_linux_app_cb.rcvPktPtr->data[0] == RSI_ENABLE)
	  {
#ifdef RSI_ENABLE_DEBUG_PRINT
        printf("\n UART FLOW CONTROL ENABLED\n");
#endif
        newtio.c_cflag |= CRTSCTS;
        tcsetattr(rsi_linux_app_cb.ttyfd, TCSANOW, &newtio);
      }
	  else
	  {
	  #ifdef RSI_ENABLE_DEBUG_PRINT
