...
//! local device name
#define RSI_BLE_APP_GATT_TEST "BLE_GATT_TEST"
...
adv:
      ...
      case RSI_BLE_GATT_READ: {
        ...
        if (loop == 0) {
          ...
          rsi_ble_set_att_value_async(app_conn_event.dev_addr, 3, 12, (uint8_t *)"mallikarjuna");
        }
        loop++;
      } break;
      ...
      case RSI_BLE_GATT_WRITE: {
        ...
        rsi_ble_set_att_cmd(remote_dev_bd_addr, 3, 12, (uint8_t *)"mallikarjuna");
      } break;
...
int main(void)
{
  ...
  rsi_task_create((rsi_task_function_t)rsi_ble_simple_gatt_test,
                  (uint8_t *)"ble_task",
