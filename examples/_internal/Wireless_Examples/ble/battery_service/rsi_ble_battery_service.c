/*******************************************************************************
* @file  rsi_ble_battery_service.c
* @brief 
*******************************************************************************
* # License
...
// To avoid unused variable warning, the 'query_serv' kept under macro, it doesn't effect the code
#if (GATT_ROLE == CLIENT)
  uuid_t query_serv = { 0 };
  uint8_t ix        = 0;
#endif
      ...
      case RSI_BLE_GATT_DESC_SERVICES: {
        ...
        for (ix = 0; ix < get_desc_services.num_of_att; ix++) {
          ...
          if (get_desc_services.att_desc[ix].att_type_uuid.val.val16 == RSI_BLE_CLIENT_CHAR_UUID) {
            ...
            rsi_ble_set_att_value(conn_event_to_app.dev_addr,
                                  *((uint16_t *)get_desc_services.att_desc[ix].handle),
                                  2,
                                  data);
          }
...
int main(void)
{
  ...
  rsi_task_create((rsi_task_function_t)rsi_ble_simple_chat_gatt_server,
                  (uint8_t *)"ble_task",
