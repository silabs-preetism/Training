...
glucose_ctrl_point_t glucose_ctrl_point_val = {
  0,           GLUCOSE_OPCODE_REPORT_REC, GLUCOSE_OPERATOR_WITHIN_RANGE, GLUCOSE_FILTER_TYPE_TIME, &time_param1,
  &time_param2
...
//! enum for global state
typedef enum { CONNECTED, ENCRYPT_EN, ADVERTISE, SCAN } app_state_t;
...
static uint16_t float_to_16bit_sfloat(float data)
{
  ...
  if (isnan(data)) {
    ...
    goto end;
  } else if (data >= -SFLOAT_EPSILON && (double)data <= SFLOAT_EPSILON) {
    result = 0;
    goto end;
  }
  ...
  while (mdiff > 0.5 && exponent > SFLOAT_EXPONENT_MIN && (mantissa_f * 10) <= SFLOAT_MANTISSA_MAX) {
    mantissa_f *= 10;
    --exponent;
    smantissa = rsi_round(mantissa_f * SFLOAT_PRECISION);
    rmantissa = rsi_round(mantissa_f) * SFLOAT_PRECISION;
...
static bool rsi_ble_ctrl_point_handle_operator(glucose_ctrl_point_t *p_ctrl_point_cmd)
{
  ...
  switch (p_ctrl_point_cmd->operator) {
    ...
    case GLUCOSE_OPERATOR_WITHIN_RANGE:
      ret = set_within_range_valid_record(p_ctrl_point_cmd->filter_type,
                                          p_ctrl_point_cmd->param1,
                                          p_ctrl_point_cmd->param2);
      break;
...
scan:
      ...
      case RSI_BLE_ENCRYPT_STARTED_EVENT: {
        ...
        rsi_ble_set_att_value(remote_dev_bd_addr, rsi_ble_ctrl_point_val_hndl, len, data);
      ...
      case RSI_BLE_GATT_CHAR_DESC_RESP_EVENT: {
        ...
        for (char_desc_index = 0; char_desc_index < att_desc.num_of_att; char_desc_index++) {
          LOG_PRINT(
            "char_handle 0x%04x, char_uuid: 0x%04x \n",
...
int main(void)
{
  ...
  rsi_task_create((rsi_task_function_t)rsi_ble_glucose_gatt_server,
                  (uint8_t *)"ble_task",
