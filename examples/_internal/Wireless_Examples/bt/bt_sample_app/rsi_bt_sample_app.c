...
#define RSI_BT_LOCAL_NAME (void *)"BT_DEV_005"
...
static uint8_t channel_map[CHANNEL_MAP_LEN] = {
  0xff, 0xff, 0xff, 0x00, 0xff, 0x00, 0x00, 0xff, 0x00, 0x0f
}; // Minimum 20 channels needed
/*==============================================*/
/**
...
void rsi_bt_app_on_conn_status_event(uint16_t resp_status, rsi_bt_event_bond_t *bond_response)
{
  ...
  LOG_PRINT("\non_conn_status: rsi_app_resp_get_bond_dev_addr: %s\r\n",
            rsi_6byte_dev_address_to_ascii(rsi_app_resp_get_bond_dev_addr, bond_response->dev_addr));
...
void rsi_bt_app_on_unbond_status_event(uint16_t resp_status, rsi_bt_event_unbond_t *unbond_status)
{
  ...
  LOG_PRINT("\non_disconn_status: rsi_app_resp_get_unbond_dev_addr: %s\r\n",
            rsi_6byte_dev_address_to_ascii(rsi_app_resp_get_unbond_dev_addr, unbond_status->dev_addr));
...
void rsi_bt_app_on_pincode_req_event(uint16_t resp_status, rsi_bt_event_user_pincode_request_t *user_pincode_request)
{
  ...
  LOG_PRINT("\non_pincode_req: rsi_app_resp_get_bond_dev_addr: %s\r\n",
            rsi_6byte_dev_address_to_ascii(rsi_app_resp_get_bond_dev_addr, user_pincode_request->dev_addr));
...
void rsi_bt_app_on_linkkey_req_event(uint16_t resp_status, rsi_bt_event_user_linkkey_request_t *user_linkkey_request)
{
  ...
  LOG_PRINT("\non_linkkey_req: rsi_app_resp_get_bond_dev_addr: %s\r\n",
            rsi_6byte_dev_address_to_ascii(rsi_app_resp_get_bond_dev_addr, user_linkkey_request->dev_addr));
...
void rsi_bt_app_on_linkkey_save_event(uint16_t status, rsi_bt_event_user_linkkey_save_t *user_linkkey_save)
{
  ...
  LOG_PRINT("\non_linkkey_save: rsi_app_resp_get_bond_dev_addr: %s\r\n",
            rsi_6byte_dev_address_to_ascii(rsi_app_resp_get_bond_dev_addr, user_linkkey_save->dev_addr));
...
void rsi_bt_app_on_auth_complete_event(uint16_t resp_status, rsi_bt_event_auth_complete_t *auth_complete)
{
  ...
  LOG_PRINT("\non_auth_complete: rsi_app_resp_get_bond_dev_addr: %s\r\n",
            rsi_6byte_dev_address_to_ascii(rsi_app_resp_get_bond_dev_addr, auth_complete->dev_addr));
  if (resp_status == 0) {
 ...
 * This callback function indicates the linkkey save from local device
 */
void rsi_bt_app_on_remote_name_cancel_resp_event(uint16_t resp_status,
                                                 rsi_bt_event_remote_name_request_cancel_t *remote_name_cancel)
{
  UNUSED_PARAMETER(resp_status); //This statement is added only to resolve compilation warning, value is unchanged
...
void rsi_bt_app_on_disconn_event(uint16_t resp_status, rsi_bt_event_disconnect_t *bt_disconnected)
{
  ...
  LOG_PRINT("\non_disconn_status: rsi_app_resp_get_disconnect_dev_addr: %s\r\n",
            rsi_6byte_dev_address_to_ascii(rsi_app_resp_get_unbond_dev_addr, bt_disconnected->dev_addr));
...
void rsi_bt_app_on_spp_connect(uint16_t resp_status, rsi_bt_event_spp_connect_t *spp_connect)
{
  ...
  LOG_PRINT("\non_spp_connect: rsi_app_resp_get_bond_dev_addr: %s\r\n",
            rsi_6byte_dev_address_to_ascii(rsi_app_resp_get_bond_dev_addr, spp_connect->dev_addr));
...
void rsi_bt_app_on_spp_disconnect(uint16_t resp_status, rsi_bt_event_spp_disconnect_t *spp_disconn)
{
  ...
  LOG_PRINT("\non_spp_disconnect: rsi_app_resp_get_bond_dev_addr: %s\r\n",
            rsi_6byte_dev_address_to_ascii(rsi_app_resp_get_bond_dev_addr, spp_disconn->dev_addr));
  ...
  while (1) {
    ...
    switch (temp_event_map) {
      case RSI_APP_EVENT_CONNECTED: {
        ...
        status = rsi_bt_get_rssi((uint8_t *)remote_bond_dev_addr, &rsi_app_resp_rssi);
        if (status != RSI_SUCCESS) {
          //return status;
        }
      ...
      case RSI_APP_EVENT_PINCODE_REQ: {
        ...
        status = rsi_bt_pincode_request_reply((uint8_t *)remote_bond_dev_addr, (uint8_t *)&pin_code, 1);
        if (status != RSI_SUCCESS) {
          //return status;
        }
#if 1
        //! Test API_15 - Get local device role: 0 - Master, 1 - Slave.
        status = rsi_bt_get_local_device_role((uint8_t *)remote_bond_dev_addr, &device_state);
        if (status != RSI_SUCCESS) {
          //return status;
        }
        ...
        status = rsi_bt_remote_name_request_async((uint8_t *)remote_bond_dev_addr, &remote_device_name);
        if (status != RSI_SUCCESS) {
          //return status;
        }
#endif
      ...
      case RSI_APP_EVENT_LINKKEY_REQ: {
        ...
        status = rsi_bt_linkkey_request_reply((uint8_t *)remote_bond_dev_addr, NULL, 0);
        if (status != RSI_SUCCESS) {
          //return status;
        }
      } break;
      ...
      case RSI_APP_EVENT_INQ_COMPLT: {
        ...
        status = rsi_bt_connect((uint8_t *)remote_bond_dev_addr);
        if (status != RSI_SUCCESS) {
          //return status;
        }
#endif
