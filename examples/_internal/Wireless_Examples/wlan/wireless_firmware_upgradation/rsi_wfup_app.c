 ...
 * @section Description :
 * The Wireless Firmware upgrade application demonstrates how SiLabs device
...
int32_t rsi_wfup_app()
{
  ...
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nFirmware Version Request Failed, Error Code : 0x%lX\r\n", status);
    return status;
  }
  ...
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\Wireless deinit Failed,Error Code is:0x%04x\r\n", status);
    return status;
  }
#ifdef RSI_M4_INTERFACE
...
int32_t wfup_loop(void)
{
  ...
  while (1) {
    ...
    if (rsp_received == RSI_FWUP_RSP_OK) {
      ...
      if (status != RSI_SUCCESS) {
        LOG_PRINT("\r\nWireless Firmware Upgrade Request Failed, Error Code : 0x%lX\r\n", status);
        return status;
      }
    ...
    if (rsp_received == RSI_FWUP_RSP_FAILED) {
      ...
      LOG_PRINT("\r\nFirmware Upgrade Failed, Error Code : 0x%lX\r\n", status);
      return status;
    }
  }
}
...
int main()
{
  ...
  rsi_task_create((rsi_task_function_t)rsi_wfup_app,
                  (uint8_t *)"wlan_task",
