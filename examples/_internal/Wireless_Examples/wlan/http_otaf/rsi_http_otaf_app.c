...
#include "deviceca.pem"
//! Server port number
#define HTTP_PORT 443
//! Server URL
#define HTTP_URL "firmware.rps"
//! Server Hostname
char *hostname = "rs9116fmw.s3.ap-south-1.amazonaws.com";
...
int32_t rsi_http_otaf_app()
{
  ...
  if (flags & HTTPS_SUPPORT) {
    ...
    status = rsi_wlan_set_certificate(RSI_SSL_CA_CERTIFICATE, deviceca, (sizeof(deviceca) - 1));
#else
  ...
  while (1) {
    ...
    switch (rsi_wlan_app_cb.state) {
      ...
      case RSI_WLAN_HTTP_OTA_WIRELESS_DEINT_STATE: {
        ...
        if (status != RSI_SUCCESS) {
          LOG_PRINT("\r\n,Wireless deint failed -Error Code : 0x%X\r\n", status);
        }
...
int main()
{
  ...
  status = rsi_task_create(rsi_http_otaf_app,
                           ...
                           &wlan_task_handle);
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nWLAN Task Creation Failed, Error Code : %d\r\n", status);
    return status;
  }
  ...
  status = rsi_task_create(rsi_wireless_driver_task,
                           ...
                           &driver_task_handle);
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nDriver Task Creation Failed, Error Code : %d\r\n", status);
    return status;
  }
  ...
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nHTTP OTAF Application Failed, Error Code : %d\r\n", status);
    return status;
  }
