...
int32_t rsi_tcp_server()
{
  ...
  status =
    rsi_setsockopt(server_socket, SOL_SOCKET, SO_TCP_KEEP_ALIVE, &tcp_keep_alive_time, sizeof(tcp_keep_alive_time));
  if (status != RSI_SUCCESS) {
  ...
  while (packet_count < NUMBER_OF_PACKETS) {
    ...
    do {
      ...
      if (status < 0) {
        ...
        rsi_shutdown(server_socket, 0);
        LOG_PRINT("\r\nFailed to Rceive data to socket, Error Code : 0x%lX\r\n", status);
        return status;
      }
...
int main()
{
  ...
  rsi_task_create((rsi_task_function_t)rsi_tcp_server,
                  (uint8_t *)"wlan_task",
