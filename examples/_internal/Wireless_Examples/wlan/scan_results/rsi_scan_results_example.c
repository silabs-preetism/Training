/*******************************************************************************
* @file  rsi_scan_results_example.c
* @brief 
*******************************************************************************
* # License
...
int32_t rsi_scan_results()
{
  ...
  /* Sort the scan results based on rssi */
  rsi_sort_scan_results_array_based_on_rssi(scan_results_array);
#ifdef RSI_ENABLE_DEBUG_PRINT
  print_results();
#endif

  while (ii < scan_results_array->num) {
    if (strncmp((const char *)scan_results_array->res[ii].ssid,
                (const char *)SSID,
                scan_results_array->res[ii].ssid_len)
        == 0)
      break;
    ii++;
  }
  ...
  status =
    rsi_wlan_scan((int8_t *)scan_results_array->res[ii].ssid, (uint8_t)scan_results_array->res[ii].channel_no, NULL, 0);
  if (status != RSI_SUCCESS) {
    return status;
  }
  //! Connect to an Access point
  status = rsi_wlan_connect((int8_t *)scan_results_array->res[ii].ssid,
                            (rsi_security_mode_t)scan_results_array->res[ii].security_mode,
                            PSK);
  if (status != RSI_SUCCESS) {
    return status;
  }
...
int main()
{
  ...
  rsi_task_create((rsi_task_function_t)rsi_scan_results,
                  (uint8_t *)"wlan_task",
