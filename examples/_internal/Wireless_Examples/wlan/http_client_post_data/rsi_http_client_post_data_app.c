 ...
 * @section Description :
 * This application demonstrates how to create SiLabs device as HTTP/HTTPS client
...
void rsi_http_get_response_handler(uint16_t status,
...
void rsi_http_post_response_handler(uint16_t status,
                                    const uint8_t *buffer,
                                    const uint16_t length,
                                    const uint32_t moredata);
...
int32_t rsi_http_client_app()
{
  ...
  status = rsi_http_client_post_async((uint8_t)flags,
  ...
  while (!end_of_data) {
    ...
    status =
      rsi_http_client_post_data((uint8_t *)file_content, (uint16_t)chunk_length, rsi_http_post_data_response_handler);
  ...
  status = rsi_http_client_get_async((uint8_t)flags,
                                     (uint8_t *)HTTP_SERVER_IP_ADDRESS,
                                     (uint16_t)HTTP_PORT,
                                     (uint8_t *)HTTP_URL,
                                     (uint8_t *)HTTP_HOSTNAME,
