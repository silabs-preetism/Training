...
int32_t rsi_websocket_client_app()
{
  ...
  if (flags & WEBSOCKET_SSL) {
    ...
    if (status != RSI_SUCCESS) {
      LOG_PRINT("\r\nLoad CA Certificate Failed, Error Code : 0x%lX\r\n", status);
      return status;
    } else {
  ...
  status = rsi_web_socket_create(flags,
                                 (uint8_t *)&server_ip_addr,
                                 (uint16_t)SERVER_PORT,
                                 (uint16_t)DEVICE_PORT,
                                 WEB_SOCKET_RESOURCE_NAME,
                                 WEB_SOCKET_HOST_NAME,
                                 &sockID,
                                 rsi_websocket_data_receive_handler);
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nWeb Socket Create Failed, Error Code : 0x%lX\r\n", status);
    return status;
  } else {
  ...
  while (packet_count < NUMBER_OF_PACKETS) {
    status = rsi_web_socket_send_async(sockID, opcode, (int8_t *)MESSAGE, strlen((const char *)MESSAGE));
    if (status < 0) {
      status = rsi_wlan_get_status();
      rsi_web_socket_close(sockID);
      LOG_PRINT("\r\nFailed to send data to Web Socket Server, Error Code : 0x%lX\r\n", status);
      return status;
    }
  ...
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nClient Web Socket Close Failed, Error code : 0x%lX\r\n", status);
    return status;
  } else {
...
int main()
{
  ...
  rsi_task_create((rsi_task_function_t)rsi_websocket_client_app,
                  (uint8_t *)"wlan_task",
