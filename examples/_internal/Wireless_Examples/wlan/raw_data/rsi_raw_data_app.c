...
void raw_data_receive_handler(uint16_t status, uint8_t *data, const uint32_t length)
{
  ...
  if (pkt_type == ARP_PKT) {
    ...
    *(uint16_t *)&send_buffer[20] = ARP_REPLY;
    ...
    if (arp_req.sender_ip_addr[0] || arp_req.sender_ip_addr[1] || arp_req.sender_ip_addr[2]
        || arp_req.sender_ip_addr[3]) {
      send_raw_pkt = 1;
    }
