...
/*AEC-CTR 256*/
uint8_t key_ctr[] = { 0x60, 0x3d, 0xeb, 0x10, 0x15, 0xca, 0x71, 0xbe, 0x2b, 0x73, 0xae, 0xf0, 0x85, 0x7d, 0x77, 0x81,
...
int32_t rsi_crypto_aes_app()
{
  ...
  memset(aes_encry_data, 0, 256); //memset response buffers
  status =
    rsi_aes(CTR_MODE, AES_ENCRYPTION, msg_ctr, sizeof(msg_ctr), key_ctr, AES_KEY_SIZE_256, iv_ctr, aes_encry_data);
  if (status != RSI_SUCCESS) {
#ifdef RSI_ENABLE_DEBUG_PRINT
...
int main()
{
  ...
  rsi_task_create(rsi_aes_encry_decry_app,
                  "aes_task",
                  RSI_AES_TASK_STACK_SIZE,
                  NULL,
                  RSI_AES_TASK_PRIORITY,
                  &aes_task_handle);
