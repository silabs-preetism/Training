...
static uint8_t spp_data[RSI_BT_MAX_PAYLOAD_SIZE];
static uint16_t spp_data_len, data_len;
...
void rsi_bt_on_passkey_display(uint16_t resp_status, rsi_bt_event_user_passkey_display_t *bt_event_user_passkey_display)
{
  ...
  LOG_PRINT("\r\n");
  LOG_PRINT("passkey: %d", (int)*((uint32_t *)bt_event_user_passkey_display->passkey));
  LOG_PRINT("\r\n");
}
...
#if (SPP_MODE == SPP_MASTER)
  status = rsi_bt_connect(rsi_ascii_dev_address_to_6bytes_rev(remote_dev_addr, (int8_t *)REMOTE_BD_ADDR));
  if (status != RSI_SUCCESS) {
    return status;
  }
  LOG_PRINT("bt_conn resp is 0x%lx \n", status);
#endif
  ...
  while (1) {
    ...
    switch (temp_event_map) {
      ...
      case RSI_APP_EVENT_SPP_CONN: {
        ...
        status                = rsi_bt_spp_transfer(remote_dev_addr, data, strlen((char *)data));
#endif
