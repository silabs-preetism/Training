...
extern uint32_t rsi_udp_start_checksum(uint8_t proto,
                                       uint16_t len,
                                       uint16_t port1,
                                       uint16_t port2,
                                       uint8_t *srcaddr,
...
extern void rsi_wlan_app_send_to_bt(uint16_t msg_type, uint8_t *data, uint16_t data_len);
      ...
      if (rsi_wlan_app_cb.event_map & RSI_WLAN_EVENT) {
        ...
        switch (pkt_type) {
          ...
          case RSI_IP_PACKET: {
            ...
            udp_recv(&rsi_wlan_app_cb.wlan_buffer[UDP_HEADER_OFFSET],
                     (rsi_wlan_app_cb.wlan_pkt_length - UDP_HEADER_OFFSET));
...
{
  UNUSED_PARAMETER(gateway);      //This statement is added only to resolve compilation warning, value is unchanged
  UNUSED_PARAMETER(network_mask); //This statement is added only to resolve compilation warning, value is unchanged
...
int32_t rsi_wlan_check_packet_type(uint8_t *buffer, uint32_t length)
{
  ...
  if (ether_header->ether_type == RSI_ARP_PACKET) {
    ...
    if ((rsi_bytes4R_to_uint32(ip_header->dstaddr) == rsi_wlan_app_cb.device_params.module_ip_addr)
        && (ip_header->proto == UDP_PROTOCOL)) {
...
{
  UNUSED_PARAMETER(remote_port); //This statement is added only to resolve compilation warning, value is unchanged
...
uint32_t rsi_udp_start_checksum(uint8_t proto,
                                uint16_t len,
                                uint16_t port1,
                                uint16_t port2,
                                uint8_t *srcaddr,
...
void prepare_udp_pkt_header(uint8_t *buffer, uint32_t length)
{
  ...
  csum      = rsi_udp_start_checksum(UDP_PROTOCOL,
                                (uint16_t)(length - IP_HEADER_OFFSET),
                                rsi_wlan_app_cb.device_params.source_port,
                                REMOTE_PORT,
                                udp_pkt->ip_header.srcaddr,
                                udp_pkt->ip_header.dstaddr);
