...
//! local device name
#define RSI_BLE_APP_DEVICE_NAME "CO-EX_MUL_SLAVES"
...
static void rsi_ble_app_init_events()
{
  ...
  UNUSED_PARAMETER(ble_app_event_mask); //To suppress warning while compiling
  return;
}
...
static void rsi_ble_on_gatt_write_event(uint16_t event_id, rsi_ble_event_write_t *rsi_ble_write)
{
  ...
  rsi_6byte_dev_address_to_ascii(remote_dev_addr, (uint8_t *)rsi_ble_write->dev_addr);
  LOG_PRINT("\n DEV_ADDR: %s\r\n", remote_dev_addr);
...
{
  UNUSED_PARAMETER(data);     //This statement is added only to resolve compilation warning, value is unchanged
  UNUSED_PARAMETER(data_len); //This statement is added only to resolve compilation warning, value is unchanged
...
int32_t rsi_ble_app_init(void)
{
  ...
  if (status != RSI_SUCCESS) {
    LOG_PRINT("\r\nBLE Start Scanning Failed, Error Code : 0x%lX\r\n", status);
    return status;
  }
        ...
        if (num_of_conn_slaves < MAX_NUM_OF_SLAVES) {
          if ((!strcmp((char *)conn_dev_addr, RSI_BLE_DEV_1_ADDR))
            ...
            status = rsi_ble_connect(
              ...
              (int8_t *)rsi_ascii_dev_address_to_6bytes_rev(remote_dev_bd_addr, (int8_t *)conn_dev_addr));
            if (status != RSI_SUCCESS) {
              return status;
            }
            conn_req_pending = 1;
...
scan:
    ...
    case RSI_BLE_SLAVE1_START_NOTIFY: {
      slave_val = 0x0001;
      rsi_ble_set_att_value_async((uint8_t *)RSI_BLE_DEV_1_ADDR, 0x60, 2, (uint8_t *)&slave_val);
    ...
    case RSI_BLE_SLAVE2_START_NOTIFY: {
      slave_val = 0x0001;
      rsi_ble_set_att_value_async((uint8_t *)RSI_BLE_DEV_2_ADDR, 0x60, 2, (uint8_t *)&slave_val);
    ...
    case RSI_BLE_SLAVE3_START_NOTIFY: {
      slave_val = 0x0001;
      rsi_ble_set_att_value_async((uint8_t *)RSI_BLE_DEV_3_ADDR, 0x60, 2, (uint8_t *)&slave_val);
    ...
    case RSI_BLE_SLAVE4_START_NOTIFY: {
      slave_val = 0x0001;
      rsi_ble_set_att_value_async((uint8_t *)RSI_BLE_DEV_4_ADDR, 0x60, 2, (uint8_t *)&slave_val);
    ...
    case RSI_BLE_SLAVE5_START_NOTIFY: {
      slave_val = 0x0001;
      rsi_ble_set_att_value_async((uint8_t *)RSI_BLE_DEV_5_ADDR, 0x60, 2, (uint8_t *)&slave_val);
    ...
    case RSI_BLE_SLAVE6_START_NOTIFY: {
      slave_val = 0x0001;
      rsi_ble_set_att_value_async((uint8_t *)RSI_BLE_DEV_6_ADDR, 0x60, 2, (uint8_t *)&slave_val);
    ...
    case RSI_BLE_SLAVE7_START_NOTIFY: {
      slave_val = 0x0001;
      rsi_ble_set_att_value_async((uint8_t *)RSI_BLE_DEV_7_ADDR, 0x60, 2, (uint8_t *)&slave_val);
    ...
    case RSI_BLE_SLAVE8_START_NOTIFY: {
      slave_val = 0x0001;
      rsi_ble_set_att_value_async((uint8_t *)RSI_BLE_DEV_8_ADDR, 0x60, 2, (uint8_t *)&slave_val);
