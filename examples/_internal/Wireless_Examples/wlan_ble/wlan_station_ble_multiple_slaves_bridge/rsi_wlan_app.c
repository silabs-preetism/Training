 ...
 * station and forward to BT task, Send data received by BT task to connected
 * station using TCP over SSL client
 =================================================================================*/
...
typedef enum rsi_app_cmd_e {
  ...
  RSI_DATA1                     = 1,
  RSI_DATA2                     = 2,
  RSI_SLAVE1_START_NOTIFICATION = 4,
  RSI_SLAVE1_STOP_NOTIFICATION  = 5,
  RSI_SLAVE2_START_NOTIFICATION = 6,
  RSI_SLAVE2_STOP_NOTIFICATION  = 7,
  RSI_SLAVE3_START_NOTIFICATION = 8,
  RSI_SLAVE3_STOP_NOTIFICATION  = 9
} rsi_app_cmd_t;
...
void rsi_wlan_async_data_recv(uint32_t sock_no, uint8_t *buffer, uint32_t length)
{
  ...
  //! send packet to ble
  rsi_wlan_app_send_to_ble(RSI_SLAVE1_START_NOTIFICATION, buffer, length);
}
