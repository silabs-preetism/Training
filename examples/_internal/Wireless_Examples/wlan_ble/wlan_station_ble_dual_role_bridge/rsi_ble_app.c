...
void rsi_ble_simple_central_on_adv_report_event(rsi_ble_event_adv_report_t *adv_report)
{
  ...
  if (((remote_addr_type == RSI_BLE_DEV_ADDR_TYPE) && ((strcmp((char *)remote_dev_addr, RSI_BLE_DEV_ADDR) == 0)))) {
    device_found = 1;
    memcpy(remote_dev_bd_addr, (uint8_t *)adv_report->dev_addr, 6);
    rsi_6byte_dev_address_to_ascii(conn_dev_addr, (uint8_t *)adv_report->dev_addr);
...
{
  rsi_6byte_dev_address_to_ascii(remote_dev_addr, (uint8_t *)resp_conn->dev_addr);
...
static void rsi_ble_on_disconnect_event(rsi_ble_event_disconnect_t *resp_disconnect, uint16_t reason)
{
  ...
  rsi_6byte_dev_address_to_ascii(remote_dev_addr, (uint8_t *)resp_disconnect->dev_addr);
...
uint32_t rsi_ble_app_init(void)
{
  ...
  rsi_6byte_dev_address_to_ascii(str_loal_addr, (uint8_t *)loal_addr);
...
adv:
    ...
    case RSI_BLE_SLAVE_START_NOTIFY: {
      ...
      rsi_ble_set_att_value_async(
        rsi_ascii_dev_address_to_6bytes_rev((uint8_t *)remote_dev_bd_addr, (int8_t *)RSI_BLE_DEV_ADDR),
        0x60,
        2,
        (uint8_t *)&slave_val);
