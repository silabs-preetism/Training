...
extern volatile uint16_t rsi_ble_att4_val_hndl;
extern volatile uint16_t rsi_ble_att5_val_hndl;
#endif
...
extern rsi_semaphore_handle_t ble_main_task_sem, ble_slave_conn_sem, bt_app_sem, wlan_app_sem, ble_scan_sem;
#if BLE_PROVISIONING_ENABLED
extern rsi_wlan_app_cb_t rsi_wlan_app_cb; //! application control block
uint8_t ble_provision_conn_id = 0xff;
uint8_t coex_ssid[50], dis_ssid[50], pwd[34], sec_type;
extern uint8_t retry;
extern uint8_t connected;
extern uint8_t ip[18];
...
#if BLE_PROVISIONING_ENABLED
//! process the data received from silabs ble provisioning app
void rsi_parse_ble_provisioning_app_data(uint8_t conn_id, rsi_ble_event_write_t *rsi_ble_write)
{
  uint8_t cmdid;
  ...
  switch (cmdid) {
    ...
    {
      //! assign the ble connection identifier
      //! wlan task uses this identifier to communicate with ble provisioning task
      ble_provision_conn_id = conn_id;
    ...
    case '2': //else if(rsi_ble_write->att_value[0] == '2')                                           //The length of the ssid and pasword is not more than 17 character
    {
      ...
      rsi_strcpy((uint8_t *)coex_ssid, &rsi_ble_write->att_value[3]);
    ...
    case '6': //else if(rsi_ble_write->att_value[0] == '6')
    {
      ...
      //memset(data,0, RSI_BLE_MAX_DATA_LEN);
      rsi_strcpy((uint8_t *)pwd, &rsi_ble_write->att_value[3]);
      rsi_wlan_app_cb.state = RSI_WLAN_JOIN_STATE;
    } break;
    ...
    case '7': //else if(rsi_ble_write->att_value[0] == '7')
    {
      ...
      if (connected) {
        //printf("\nconnected\n");
        rsi_ble_set_event_based_on_conn(conn_id, RSI_WLAN_ALREADY);
      } else {
        rsi_ble_set_event_based_on_conn(conn_id, RSI_WLAN_NOT_ALREADY);
      }
    } break;
    ...
    case '8': {
      ...
      //memset(data,0, RSI_BLE_MAX_DATA_LEN);
      rsi_ble_set_event_based_on_conn(conn_id, RSI_APP_FW_VERSION);
    } break;
 ...
 */
void rsi_wlan_app_send_to_ble(uint8_t conn_id, uint16_t msg_type, uint8_t *data, uint16_t data_len)
{
  if ((conn_id != 0xff) && (conn_id < TOTAL_CONNECTIONS)) {
    switch (msg_type) {
      case RSI_WLAN_SCAN_RESP:
        memset(&scanresult, 0, sizeof(scanresult));
        memcpy(&scanresult, (rsi_rsp_scan_t *)data, data_len);
        rsi_ble_set_event_based_on_conn(conn_id, RSI_BLE_WLAN_SCAN_RESP);
        break;
      case RSI_WLAN_CONN_STATUS:
        rsi_ble_set_event_based_on_conn(conn_id, RSI_BLE_WLAN_JOIN_STATUS);
        break;
      case RSI_WLAN_DISCONN_STATUS:
        rsi_ble_set_event_based_on_conn(conn_id, RSI_BLE_WLAN_DISCONNECT_STATUS);
        break;
      case RSI_WLAN_DISCONN_NOTIFY:
        rsi_ble_set_event_based_on_conn(conn_id, RSI_BLE_WLAN_DISCONN_NOTIFY);
        break;
      case RSI_WLAN_TIMEOUT_NOTIFY:
        rsi_ble_set_event_based_on_conn(conn_id, RSI_BLE_WLAN_TIMEOUT_NOTIFY);
        break;
      default: {
      }
    }
...
#if BLE_PROVISIONING_ENABLED
  uint8_t ble_prov_app_data[RSI_BLE_MAX_DATA_LEN] = { 0 };
#endif
  ...
  while (1) {
    ...
    switch (event_id) {
      ...
      case RSI_BLE_GATT_WRITE_EVENT: {
        ...
        if ((*(uint16_t *)(rsi_ble_conn_info[l_conn_id].app_ble_write_event.handle)) == rsi_ble_att1_val_hndl) {
          ...
          if ((err == 0) && ((rsi_ble_conn_info[l_conn_id].app_ble_write_event.length) <= attribute->max_value_len)) {
            ...
            rsi_parse_ble_provisioning_app_data(l_conn_id, &rsi_ble_conn_info[l_conn_id].app_ble_write_event);
#endif
      ...
      case RSI_APP_FW_VERSION: {
        ...
        rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_APP_FW_VERSION);
        memset(ble_prov_app_data, 0, MAX_DATA_LENGTH1);
        status               = rsi_wlan_get(RSI_FW_VERSION, resp_buf, 18);
        ble_prov_app_data[0] = 0x08;
        ble_prov_app_data[1] = 8;
        memcpy(&ble_prov_app_data[2], resp_buf, 18);

        rsi_ble_set_local_att_value(rsi_ble_att4_val_hndl, MAX_DATA_LENGTH1, ble_prov_app_data);
        LOG_PRINT("fw version returned,response on 1bb1 for '8' command\n\n");
      } break;
      // Connected SSID name (response to '7' command if connection is already established)
      case RSI_WLAN_ALREADY: {
        rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_WLAN_ALREADY);

        memset(ble_prov_app_data, 0, MAX_DATA_LENGTH1);

        ble_prov_app_data[1] = strlen((const char *)&ble_prov_app_data[2]);
        ble_prov_app_data[0] = 0x07;
        rsi_ble_set_local_att_value(rsi_ble_att4_val_hndl, MAX_DATA_LENGTH1, ble_prov_app_data);
      } break;

      // NO WLAN connection (response to '7' command if connection is there already)
      case RSI_WLAN_NOT_ALREADY: {
        rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_WLAN_NOT_ALREADY);
        memset(ble_prov_app_data, 0, MAX_DATA_LENGTH1);
        ble_prov_app_data[0] = 0x07;
        ble_prov_app_data[1] = 0x00;
        rsi_ble_set_local_att_value(rsi_ble_att4_val_hndl, MAX_DATA_LENGTH1, ble_prov_app_data);
      } break;
      case RSI_BLE_WLAN_DISCONN_NOTIFY: {
        rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_BLE_WLAN_DISCONN_NOTIFY);
        memset(ble_prov_app_data, 0, MAX_DATA_LENGTH1);
        ble_prov_app_data[1] = 0x01;
        ble_prov_app_data[0] = 0x04;
        rsi_ble_set_local_att_value(rsi_ble_att4_val_hndl, MAX_DATA_LENGTH1, ble_prov_app_data);
      } break;
      case RSI_BLE_WLAN_TIMEOUT_NOTIFY: {
        rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_BLE_WLAN_TIMEOUT_NOTIFY);
        memset(ble_prov_app_data, 0, MAX_DATA_LENGTH1);
        ble_prov_app_data[0] = 0x02;
        ble_prov_app_data[1] = 0x00;
        rsi_ble_set_local_att_value(rsi_ble_att4_val_hndl, MAX_DATA_LENGTH1, ble_prov_app_data);
      } break;

      case RSI_BLE_WLAN_DISCONNECT_STATUS: {
        rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_BLE_WLAN_DISCONNECT_STATUS);
        memset(ble_prov_app_data, 0, MAX_DATA_LENGTH1);
        ble_prov_app_data[0] = 0x01;
        rsi_ble_set_local_att_value(rsi_ble_att4_val_hndl, MAX_DATA_LENGTH1, ble_prov_app_data);
      } break;
      ...
      {
        rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_BLE_WLAN_SCAN_RESP); //! clear the served event
        memset(ble_prov_app_data, 0, MAX_DATA_LENGTH1);
        ble_prov_app_data[0] = 0x03;
        ble_prov_app_data[1] = scanresult.scan_count[0];
        rsi_ble_set_local_att_value(rsi_ble_att4_val_hndl, MAX_DATA_LENGTH1, ble_prov_app_data);
        LOG_PRINT("No.of scan list,response on 1bb1 for '3' command\n");
        for (uint8_t scan_ix = 0; scan_ix < scanresult.scan_count[0]; scan_ix++) {
          memset(ble_prov_app_data, 0, MAX_DATA_LENGTH1);
          ble_prov_app_data[0] = scanresult.scan_info[scan_ix].security_mode;
          ble_prov_app_data[1] = ',';
          rsi_strcpy((uint8_t *)(ble_prov_app_data + 2), scanresult.scan_info[scan_ix].ssid);
          //length = strlen(ble_prov_app_data+2);
          //length = length+2;

          rsi_ble_set_local_att_value(rsi_ble_att5_val_hndl, MAX_DATA_LENGTH1, ble_prov_app_data);
          rsi_delay_ms(10);
        }
        LOG_PRINT("displayed scan list, response on 1cc1 for '3' command\n\n");
      } break;
      ...
      {
        uint8_t k = 0;
        rsi_ble_clear_event_based_on_conn(l_conn_id, RSI_BLE_WLAN_JOIN_STATUS);
        ...
        for (k = 0; k < 6; k++) {
          ble_prov_app_data[k + 3] = ip[k];
        }
        ble_prov_app_data[k + 3] = ',';

        for (; k < 10; k++) {
          ble_prov_app_data[k + 4] = ip[k];
        }

        rsi_ble_set_local_att_value(rsi_ble_att4_val_hndl,
                                    MAX_DATA_LENGTH1,
                                    ble_prov_app_data); //! set the local attribute value.

        LOG_PRINT("AP joined successfully,response on 1bb1 for '2' command\n\n");
      } break;
#endif
      default:
        break;
