...
volatile uint16_t rsi_ble_att4_val_hndl;
volatile uint16_t rsi_ble_att5_val_hndl;
#endif
...
uint8_t rsi_check_dev_list(uint8_t *remote_dev_name, uint8_t *adv_dev_addr)
{
  ...
  USED_PARAMETER(adv_dev_addr); //This statement is added only to resolve compilation warning, value is unchanged
#else
  uint8_t *adv_dev_addr1 = adv_dev_addr;
  USED_PARAMETER(remote_dev_name); //This statement is added only to resolve compilation warning, value is unchanged
#endif
...
static uint32_t rsi_ble_add_simple_chat_serv(void)
{
...
#if BLE_PROVISIONING_ENABLED
  uint8_t data[MAX_DATA_LENGTH1] = { 1, 0 };
#else
  ...
  //! adding characteristic value attribute to the service
  rsi_ble_att4_val_hndl = new_serv_resp.start_handle + 5; //! adding characteristic value attribute to the service
  new_uuid.size         = 2;
  new_uuid.val.val16    = RSI_BLE_ATTRIBUTE_2_UUID;
  rsi_ble_add_char_val_att(new_serv_resp.serv_handler,
                           new_serv_resp.start_handle + 5,
  ...
  //! adding characteristic value attribute to the service
  rsi_ble_att5_val_hndl = new_serv_resp.start_handle + 7; //! adding characteristic value attribute to the service
  new_uuid.size         = 2;
  new_uuid.val.val16    = RSI_BLE_ATTRIBUTE_3_UUID;
  rsi_ble_add_char_val_att(new_serv_resp.serv_handler,
                           new_serv_resp.start_handle + 7,
...
//! dont wait for wlan connection when ble provisioning is enabled
#if (WLAN_SYNC_REQ && !BLE_PROVISIONING_ENABLED)
  //! FIXME: Workaround
  if (rsi_wlan_running) {
