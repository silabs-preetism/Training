...
void rsi_wlan_stats_receive_handler(uint16_t status, const uint8_t *buffer, const uint32_t length)
{
  ...
  if (cmd7.no_of_stats != 0) {
    ...
    sprintf((send_buffer + strlen(send_buffer)),
            ...
            "%.4d"
            "%.4d",
            per_stats_rsp->crc_pass,
            per_stats_rsp->crc_fail,
            per_stats_rsp->cal_rssi);
...
int32_t rsi_bt_get_ber_pkt_from_app_to_buff(uint8_t *pkt, uint16_t pkt_len)
{
  ...
  if (cmdH.no_bt_pkts != 0) {
    printf("%.2x%.4x",
           pkt_id,
           (pkt_len + 3)); //sending packet index, pkt_len+header length(packet index 1byte + pkt_len 2bytes) over UART
...
int32_t rsi_send_per_features(void)
{
  ...
  rsi_feature_frame =
    (rsi_feature_frame_t *)pkt->data; //! Take the user provided data and fill it in feature frame structure
  ...
  status =
    rsi_driver_common_send_cmd(RSI_COMMON_REQ_FEATURE_FRAME, pkt); //! send features frame command to Wireless Processor
...
{
  int32_t status = RSI_SUCCESS;
  status         = rsi_ble_per_transmit((void *)&cmd8); //start tranmitting requested BLE packets
  if (status != RSI_SUCCESS) {
...
{
  int32_t status = RSI_SUCCESS;
  status         = rsi_ble_per_transmit((void *)&cmd8); //stop transmitting BLE packets
  if (status != RSI_SUCCESS) {
...
{
  int32_t status = RSI_SUCCESS;
  status         = rsi_ble_per_receive((void *)&cmd9); //start receiving BLE packets
  if (status != RSI_SUCCESS) {
...
{
  int32_t status = RSI_SUCCESS;
  status         = rsi_ble_per_receive((void *)&cmd9); //stop receiving BLE packets
  if (status != RSI_SUCCESS) {
...
void rsi_ble_stats_receive(void)
{
  ...
  do {
    while ((rsi_hal_gettickcount() - timer_1s) < 994)
      ; // waiting for 1 second for each BLE stat request
...
int32_t rsi_bt_receive_start(void)
{
  ...
  rsi_bt_common_register_callbacks(
    (void *)rsi_bt_get_ber_pkt_from_app_to_buff); //! Register BT receive stats call back handler
...
void rsi_bt_stats_receive(void)
{
  ...
  while ((rsi_hal_gettickcount() - timer_1s) < 994)
    ; // wait for maximum time to receive Bt packets
